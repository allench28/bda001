...
from constant import get_claude_anthropic_media_type
...
@tracer.capture_method
def checkCompanyImageFooter(prompt_data, file_data=None, file_type=None):
    # Create message content with both text and file data if provided
    content = create_image_checking_content(prompt_data, file_data, file_type)
...
@tracer.capture_method
def create_image_checking_content(prompt_data, image_data=None, file_extension=None):
    content = []

    if image_data and file_extension:
        media_type = get_claude_anthropic_media_type(file_extension)
        if media_type:
            # Add file content with appropriate media type
            content.append({
                "type": "image",
                "source": {
                    "type": "base64",
