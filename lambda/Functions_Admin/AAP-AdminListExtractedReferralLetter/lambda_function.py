...
def getDataFromES(merchantId, sortField, sortDirection, filters, limit, nextToken): #childrenMerchantIdList
    ...
    url = 'https://' + ES_DOMAIN + "/extractedreferralletter/_doc/_search"
...
def getCurrentESLimit():
    esUrl = f'https://{ES_DOMAIN}/extractedreferralletter/_settings'
    headers = {
            'Content-Type': "application/json",
            'User-Agent': "PostmanRuntime/7.20.1",
            'Accept': "*/*",
    ...
    responseText = json.loads(response.text)
    maxResultWindow = responseText.get('extractedreferralletter').get('settings').get('index').get('max_result_window')
