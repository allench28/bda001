...
EXTRACTED_GRN_LINE_ITEM_DDB_TABLE = DDBRESOURCE.Table(EXTRACTED_GRN_LINE_ITEMS_TABLE)
...
def query_extracted_grn_line_items(extractedGrnId):
    response = EXTRACTED_GRN_LINE_ITEM_DDB_TABLE.query(
        IndexName='gsi-extractedGrnId',
        KeyConditionExpression=Key('extractedGrnId').eq(extractedGrnId),
    )
...
def formatResponse(data):
    for item in data:
        ...
        extracted_documets = query_extracted_grn_line_items(item.get('extractedGrnId'))
...
def getDataFromES(merchantId, sortField, sortDirection, filters, limit, nextToken): #childrenMerchantIdList
    ...
    url = 'https://' + ES_DOMAIN + "/extractedgrn/_doc/_search"
...
def getCurrentESLimit():
    esUrl = f'https://{ES_DOMAIN}/extractedgrn/_settings'
    headers = {
            'Content-Type': "application/json",
            'User-Agent': "PostmanRuntime/7.20.1",
            'Accept': "*/*",
    ...
    responseText = json.loads(response.text)
    maxResultWindow = responseText.get('extractedgrn').get('settings').get('index').get('max_result_window')
