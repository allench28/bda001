...
EXTRACTED_DOCUMENTS_DDB_TABLE = DDBRESOURCE.Table(EXTRACTED_DOCUMENTS_LINE_ITEMS_TABLE)
...
def lambda_handler(event, context):
    try:
        ...
        tracer.put_annotation("lamb     da_error", "true")
...
def query_extracted_documents_line_items(extractedDocumentsId):
    response = EXTRACTED_DOCUMENTS_DDB_TABLE.query(
        IndexName='gsi-extractedDocumentsId',
        KeyConditionExpression=Key('extractedDocumentsId').eq(extractedDocumentsId),
    )
...
def formatResponse(data):
    for item in data:
        documentUploadId = item.get('documentUploadId')
        documentUpload = getDocumentUpload(documentUploadId)
        if documentUpload:
            item['fileName'] = documentUpload.get('fileName')
            item['inputPath'] = documentUpload.get('inputPath')
            
        extracted_documets = query_extracted_documents_line_items(item.get('extractedDocumentsId'))
        item['documentLineItemList'] = extracted_documets
        del item['merchantId']
...
def getDataFromES(merchantId, sortField, sortDirection, filters, limit, nextToken): #childrenMerchantIdList
    ...
    url = 'https://' + ES_DOMAIN + "/extracteddocuments/_doc/_search"
...
def getCurrentESLimit():
    esUrl = f'https://{ES_DOMAIN}/extracteddocuments/_settings'
    headers = {
            'Content-Type': "application/json",
            'User-Agent': "PostmanRuntime/7.20.1",
            'Accept': "*/*",
    ...
    responseText = json.loads(response.text)
    maxResultWindow = responseText.get('extracteddocuments').get('settings').get('index').get('max_result_window')
