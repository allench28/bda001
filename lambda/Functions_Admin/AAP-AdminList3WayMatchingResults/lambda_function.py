...
THREE_WAY_MATCHING_DDB_TABLE = DDBRESOURCE.Table(THREE_WAY_MATCHING_RESULTS_TABLE)
...
def lambda_handler(event, context):
    try:
        ...
        data, totalData = getDataFromES(merchant_id, sortField, sortDirection, filters, limit, nextToken) # childrenMerchantIdList
...
def query_three_way_matching_result_items(invoiceNumber):
    response = THREE_WAY_MATCHING_DDB_TABLE.query(
        IndexName='gsi-invoiceNumber',
        KeyConditionExpression=Key('invoiceNumber').eq(invoiceNumber),
    )
...
def formatResponse(data):
    for item in data:
        threeWayMatchingResultsId = item.get('threeWayMatchingResultsId')
        threeWayMatchingResultsId = getthreeWayMatchingResultsId(threeWayMatchingResultsId)
        if threeWayMatchingResultsId:
            item['fileName'] = threeWayMatchingResultsId.get('fileName')
            item['inputPath'] = threeWayMatchingResultsId.get('inputPath')
            
        three_way_matching_results = query_three_way_matching_result_items(item.get('invoiceNumber'))
        item['threeWayMatchingResultList'] = three_way_matching_results
        del item['merchantId']

    return data

@tracer.capture_method
def getDataFromES(merchantId, sortField, sortDirection, filters, limit, nextToken): #childrenMerchantIdList
    filterConditionMap = {
            'eq': 'match_phrase',
            'match': 'match',
    ...
    url = 'https://' + ES_DOMAIN + "/threewaymatchingresults/_doc/_search"
    # merchantIdsInFilter = getMerchantIdFromFilter(filters)
    # if not merchantIdsInFilter:
    #     merchantIdsInFilter = childrenMerchantIdList
    # merchantIds = list(set(merchantIdsInFilter) & set(childrenMerchantIdList))
...
def getCurrentESLimit():
    esUrl = f'https://{ES_DOMAIN}/threewaymatchingresults/_settings'
    headers = {
            'Content-Type': "application/json",
            'User-Agent': "PostmanRuntime/7.20.1",
            'Accept': "*/*",
    ...
    responseText = json.loads(response.text)
    maxResultWindow = responseText.get('threewaymatchingresults').get('settings').get('index').get('max_result_window')
...
@tracer.capture_method
def getthreeWayMatchingResultsId(threeWayMatchingResultsId):
    response = THREE_WAY_MATCHING_DDB_TABLE.get_item(
        Key={'threeWayMatchingResultsId': threeWayMatchingResultsId}
    ).get('Item', {})
    return response
