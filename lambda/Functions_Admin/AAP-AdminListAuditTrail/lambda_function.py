...
TIMELINE_DDB_TABLE = DDBRESOURCE.Table(TIMELINE_TABLE)
...
@tracer.capture_method
def getDataFromES(merchantId, sortField, sortDirection, filters, limit, nextToken):
    filterConditionMap = {
            'eq': 'match_phrase',
            'match': 'match',
    ...
    url = 'https://' + ES_DOMAIN + "/timeline/_doc/_search"
...
def getCurrentESLimit():
    esUrl = f'https://{ES_DOMAIN}/timeline/_settings'
    headers = {
            'Content-Type': "application/json",
            'User-Agent': "PostmanRuntime/7.20.1",
            'Accept': "*/*",
    ...
    responseText = json.loads(response.text)
    maxResultWindow = responseText.get('timeline').get('settings').get('index').get('max_result_window')
