...
EXTRACTED_PO_LINE_ITEM_DDB_TABLE = DDBRESOURCE.Table(EXTRACTED_PO_LINE_ITEMS_TABLE)
...
def query_extracted_po_line_items(extractedPoId):
    response = EXTRACTED_PO_LINE_ITEM_DDB_TABLE.query(
        IndexName='gsi-extractedPoId',
        KeyConditionExpression=Key('extractedPoId').eq(extractedPoId),
    )
...
def formatResponse(data):
    for item in data:
        ...
        extracted_documets = query_extracted_po_line_items(item.get('extractedPoId'))
...
def getDataFromES(merchantId, sortField, sortDirection, filters, limit, nextToken): #childrenMerchantIdList
    ...
    url = 'https://' + ES_DOMAIN + "/extractedpo/_doc/_search"
...
def getCurrentESLimit():
    esUrl = f'https://{ES_DOMAIN}/extractedpo/_settings'
    headers = {
            'Content-Type': "application/json",
            'User-Agent': "PostmanRuntime/7.20.1",
            'Accept': "*/*",
    ...
    responseText = json.loads(response.text)
    maxResultWindow = responseText.get('extractedpo').get('settings').get('index').get('max_result_window')
